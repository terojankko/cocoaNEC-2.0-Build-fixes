...
- (void)awakeFromNib
{
	...
	output = [ [ NECOutput alloc ] init ] ;
 	[ animateMenuItem setState:NSControlStateValueOn ] ;
...
- (void)setEngineType
{
	...
	case 1:
		...
		return ;
	case 2:
		engineType = ( [ useGN2Checkbox state ] == NSControlStateValueOff ) ? kNEC42Engine : kNEC42EngineGN2 ;		//  v0.78, v0.80
        [ Config setEngineType:engineType ] ;       //  v0.89
		return ;
	}
...
- (void)setCurrentNCSystem:(NCSystem*)sys
{
    ...
    if (![NSThread isMainThread]) {
        NSLog(@"setCurrentNCSystem from non-main thread");
    }
}
...
- (IBAction)setAsReference:(id)sender
{
	...
	if ( menuItem ) [ menuItem setState:NSControlStateValueOff ] ;
...
- (void)polOff
{
	[ polMenu1 setState:NSControlStateValueOff ] ;
	[ polMenu2 setState:NSControlStateValueOff ] ;
	[ polMenu3 setState:NSControlStateValueOff ] ;
	[ polMenu4 setState:NSControlStateValueOff ] ;
	[ polMenu5 setState:NSControlStateValueOff ] ;
	[ polMenu6 setState:NSControlStateValueOff ] ;
	[ polMenu7 setState:NSControlStateValueOff ] ;
}
...
- (void)setPolarizationMenu:(intType)pol
{
	...
	switch ( pol ) {
	case 0:	
		[ polMenu2 setState:NSControlStateValueOn ] ;
		break ;
	case 1:	
		[ polMenu3 setState:NSControlStateValueOn ] ;
		break ;
	case 2:	
		[ polMenu1 setState:NSControlStateValueOn ] ;
		break ;
	case 3:	
		[ polMenu5 setState:NSControlStateValueOn ] ;
		break ;
	case 4:	
		[ polMenu6 setState:NSControlStateValueOn ] ;
		break ;
	case 5:	
		[ polMenu4 setState:NSControlStateValueOn ] ;
		break ;
	case 6:	
		[ polMenu7 setState:NSControlStateValueOn ] ;
		break ;
	}
}
...
{
	[ sender setState:( [ sender state ] == NSControlStateValueOn ) ? NSControlStateValueOff : NSControlStateValueOn ] ;
	enabled3d = ( [ sender state ] == NSControlStateValueOn ) ;
}
...
- (void)updatePrefs
{
	...
	if ( dict ) {
		...
		if ( number != nil ) {
			[ useGN2Checkbox setState:( [ number boolValue ] ? NSControlStateValueOn : NSControlStateValueOff ) ] ;
		}
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)sender
{
	...
	[ plist setObject:[ NSNumber numberWithBool:( [ useGN2Checkbox state ] == NSControlStateValueOn ) ] forKey:kGroundPref ] ;						//  v0.80
	if ( currentNC != nil ) {
...
- (BOOL)validateUserInterfaceItem:(id<NSValidatedUserInterfaceItem>)item
{
	...
	if ( [(id)item menu] == outputMenu ) {
		return ( [ output hasModel ] ) ;
	}
	
	if ( [(id)item menu] == modelMenu ) {

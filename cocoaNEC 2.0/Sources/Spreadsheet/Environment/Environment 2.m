...
- (NSArray*)frequencyArray
{
	...
	case 1:
		...
		if ( [ linearCheckBox state ] == NSControlStateValueOn ) {
			// linear
			if ( n == 1 ) incr = 0.0 ; 
			else {
...
- (void)updateGroundType:(Boolean)isPerfectGround
{
	...
	if ( isPerfectGround ) {
		...
		if ( [ sommerfeldCheckbox state ] == NSControlStateValueOn ) groundType = 2 ;
	}
...
- (NSMutableDictionary*)makeDictionaryForPlist 
{
	...
	[ plist setObject:[ NSNumber numberWithBool:( [ linearCheckBox state ] == NSControlStateValueOn ) ] forKey:@"linear" ] ;
...
- (void)restoreFromDictionary:(NSDictionary*)dict
{
	...
	[ linearCheckBox setState:( state ) ? NSControlStateValueOn : NSControlStateValueOff ] ;

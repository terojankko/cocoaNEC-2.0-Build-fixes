...
- (WireInfo*)info
{
	...
	case 1:
		...
		// for ( i = 0; i < 2; i++ ) info.excitationVector[i] = [  (ApplicationDelegate *) [ NSApp delegate ] doubleValueForObject:[ exVoltageMatrix cellAtRow:i column:0 ] ] ;
            for (i = 0; i < 2; i++) {
                info.excitationVector[i] = [((ApplicationDelegate *) [NSApp delegate]) doubleValueForObject:[exVoltageMatrix cellAtRow:i column:0]];
            }
		break ;
	case 2:
		//  current
		...
		// for ( i = 0; i < 2; i++ ) info.excitationVector[i] = [  (ApplicationDelegate *) [ NSApp delegate ] doubleValueForObject:[ currentMatrix cellAtRow:i column:0 ] ] ;
            for (i = 0; i < 2; i++) {
                info.excitationVector[i] = [((ApplicationDelegate *) [NSApp delegate]) doubleValueForObject:[currentMatrix cellAtRow:i column:0]];
            }
		break ;
	}
	return &info ;

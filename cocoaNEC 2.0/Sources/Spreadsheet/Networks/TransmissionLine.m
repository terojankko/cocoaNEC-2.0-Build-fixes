...
- (Boolean)ncCode:(NSMutableString*)code eval:(Spreadsheet*)client networkRow:(int)row
{
	...
	crossed = ( [ tlCrossedButton state ] == NSControlStateValueOn ) ;
...
- (NSString*)networkCard:(Expression*)e spreadsheet:(Spreadsheet*)client networkRow:(int)row
{
	...
	z0 = [ self evalDoubleAsString:tlMatrix row:0 cellName:"transmission line Z0" negate:( [ tlCrossedButton state ] == NSControlStateValueOn ) ] ;
...
- (NSMutableDictionary*)makePlistDictionary
{
	...
	[ plist setValue:[ NSNumber numberWithBool:( [ tlCrossedButton state ] == NSControlStateValueOn ) ] forKey:@"crossed" ] ;
	return plist ;
}
...
- (void)restoreFromDictionary:(NSDictionary*)dict
{
	...
	[ tlCrossedButton setState:( [ [ dict valueForKey:@"crossed" ] boolValue ] ) ? NSControlStateValueOn : NSControlStateValueOff ] ;
}

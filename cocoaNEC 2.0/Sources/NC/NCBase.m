...
- (void)dispatchEngine
{
    runResult =  *(RunInfo *) [(ApplicationDelegate *)  [ NSApp delegate ] runNECEngine:inputPath output:outputPath sourcePath:stack.sourcePath useQuad:[ [ compiler system ] useQuadPrecision ] ] ;
...
- (int)runWorkFlowCompile:(Boolean)doCompile execute:(Boolean)doExecute allowLoops:(Boolean)allowLoops runNEC:(Boolean)doRun sourceString:(NSString*)sourceString
{
	...
	if ( doCompile ) {
        ...
        NSAssert(success == YES, @"Compiler.precompile failed");
		success = ( [ compiler compile ] && success == YES ) ;
        NSAssert(success == YES, @"Compiler.compile failed");

...
- (void)refreshGeometry
{
	...
	//  v0.81d
	geometryOptions.radials = ( [ drawRadialsCheckbox state ] == NSControlStateValueOn ) ;
	geometryOptions.distributedLoads = ( [ drawDistributedLoadsCheckbox state ] == NSControlStateValueOn ) ;
...
- (IBAction)printView:(id)sender
{
	...
	case kCardsTab:
		if ( cardsTable ) {
            ...
            [ printInfo setHorizontalPagination:NSPrintingPaginationModeAutomatic ] ;	//  discard right side of table view that extends more than 80 columns
...
- (Boolean)drawBorders
{
	return ( [ drawBordersCheckbox state ] == NSControlStateValueOn ) ;
}

- (Boolean)drawBackgrounds
{
	return ( [ drawBackgroundsCheckbox state ] == NSControlStateValueOn ) ;
}

- (Boolean)drawFilenames
{
	return ( [ drawFilenamesCheckbox state ] == NSControlStateValueOn ) ;
}
...
- (void)updatePrefsFromDict:(NSDictionary*)plist 
{
	...
	if ( dict ) {
		...
		if ( state ) [ drawBordersCheckbox setState:( [ state boolValue ] ? NSControlStateValueOn : NSControlStateValueOff ) ] ;
		state = [ dict objectForKey:kDrawBackgrounds ] ;
		if ( state ) [ drawBackgroundsCheckbox setState:( [ state boolValue ] ? NSControlStateValueOn : NSControlStateValueOff ) ] ;
		state = [ dict objectForKey:kDrawFilenames ] ;
		if ( state ) [ drawFilenamesCheckbox setState:( [ state boolValue ] ? NSControlStateValueOn : NSControlStateValueOff ) ] ;
		...
		if ( number != nil ) {
			[ drawDistributedLoadsCheckbox setState:( [ number boolValue ] ? NSControlStateValueOn : NSControlStateValueOff ) ] ;
		}
...
- (void)savePrefsToPlist:(NSMutableDictionary*)plist
{
	...
	if ( dict ) {
		...
		//  v0.70 printing options
		[ dict setObject:[ NSNumber numberWithBool:( [ drawBordersCheckbox state ] == NSControlStateValueOn ) ] forKey:kDrawBorders ] ;
		[ dict setObject:[ NSNumber numberWithBool:( [ drawBackgroundsCheckbox state ] == NSControlStateValueOn ) ] forKey:kDrawBackgrounds ] ;
		[ dict setObject:[ NSNumber numberWithBool:( [ drawFilenamesCheckbox state ] == NSControlStateValueOn ) ] forKey:kDrawFilenames ] ;
		//  v0.70 save colors
		...
		[ dict setObject:[ NSNumber numberWithBool:( [ drawDistributedLoadsCheckbox state ] == NSControlStateValueOn ) ] forKey:kDrawDistributedLoads ] ;	//  v0.81d

...
//  v0.88  use PDFConsumer:fillDisk
- (void)drawPointAtFrequency:(float)f value:(float)value radius:(float)r color:(NSColor*)color into:(PDFConsumer*)p
{
    float x, y ;
    ...
    [ p setRGB:color ] ;
    [ p fillDisk:r x:x y:y ] ;
}

//  v0.88  use PDFConsumer:fillDisk
- (void)drawSelectedPointAtFrequency:(float)f value:(float)value radius:(float)r color:(NSColor*)color into:(PDFConsumer*)p
{
    float x, y ;
    x = [ self xFromFrequency:f ] ;
    y = [ self yFromValue:value ] ;
	...
	if ( outputContext != 0 ) {
		...
		if ( plotType == kRXPlotType ) {
			...
			for ( j = 0; j < n; j++ ) {
                ...
                [ self drawPointAtFrequency:x value:y radius:3.0 color:alternatePlotColor into:p ] ;           //  v0.88 (used to use CGContext -drawPoint:x:y in DataView)
			}
		}
		...
		for ( j = 0; j < n; j++ ) {
            ...
            rxf[j].viewLocation = NSMakePoint( [ self xFromFrequency:x ], [ self yFromValue:y ] ) ;
			if ( current->frequency < 0.001 || j != current->frequencyIndex ) {
                [ self drawPointAtFrequency:x value:y radius:3.5 color:plotColor into:p ] ;           //  v0.88 (used to use CGContext -drawPoint:x:y in DataView)
			}
			else {
				//  draw donut for feedpoint's selected frequency
                [ self drawSelectedPointAtFrequency:x value:y radius:3.5 color:plotColor into:p ] ;   //  v0.88 (used to use CGContext -drawPoint:x:y in DataView)
			}
		}
...
- (void)drawRect:(NSRect)rect
{
	...
	if ( isScreen ) {
		//  draw background for scalarView when on a display
        hasBackground = YES ;
	...
	[ canvas newpath ] ;
	[ canvas rect:clipBounds.origin.x y:clipBounds.origin.y width:width height:height ] ;
	[ canvas closepath ] ;
	[ canvas clip ] ;

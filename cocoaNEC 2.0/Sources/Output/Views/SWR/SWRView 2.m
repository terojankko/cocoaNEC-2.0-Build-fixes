			...
			for ( i = k0; i < kn; i++ ) {
				if ( i >= 0 && i < MAXFEEDPOINTS ) {
					...
					if ( samples > 3 && [ [ outputObject swrView ] doInterpolate ] ) {
						...
						frequencyGap = ( [ smartInterpolationCheckbox state ] == NSControlStateValueOn ) ? [ interpolate frequencyGap:rxf ] : 1e12 ;
...
{
	return ( [ showAllCheckbox state ] == NSControlStateValueOn ) ;
}
...
{
	return ( [ smartInterpolationCheckbox state ] == NSControlStateValueOn ) ;
}

//	v0.73
- (void)setSmartInterpolate:(Boolean)state
{
	[ smartInterpolationCheckbox setState:( state ) ? NSControlStateValueOn : NSControlStateValueOff ] ;
}
